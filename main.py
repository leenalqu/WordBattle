...
class GameProgress:
    ...
    def handle_button_click(self, mouse_x, mouse_y):
        ...
        # Click sound button.
        if (self.x_min_click_sound_button <= mouse_x <= self.x_max_click_sound_button and
                self.y_min_click_sound_button <= mouse_y <=
                self.y_max_click_sound_button):
        ...
        if (self.x_min_background_music_button <= mouse_x <=
                self.x_max_background_music_button and
                self.y_min_background_music_button <= mouse_y <=
                self.y_max_background_music_button):
            if pygame.mouse.get_pressed()[0] and self.show_options_page:
                self.button_sound.play()
        ...
        # Quit button.
        if (self.x_min_quit_button <= mouse_x <= self.x_max_quit_button and
                self.y_min_quit_button <= mouse_y <= self.y_max_quit_button and
                not (self.show_welcome_page or self.show_rules_page or
                     self.show_options_page or self.show_credits_page or
                     self.show_computer_first_page or self.show_player_first_page or
                     self.show_remove_page or self.show_remove_page_mode or
                     self.show_victory_page or self.show_defeat_page or
                     self.show_rules_main_page)):
        ...
        # Confirm button.
        if (self.x_min_confirm_button <= mouse_x <= self.x_max_confirm_button and
                self.y_min_confirm_button <= mouse_y <= self.y_max_confirm_button):
            if (pygame.mouse.get_pressed()[0] and self.side_status == 0 and
                    not (self.show_welcome_page or self.show_rules_page or
                         self.show_options_page or self.show_credits_page or
                         self.show_computer_first_page or
                         self.show_player_first_page)):
                self.button_sound.play()
                self.timer_seconds = 0
        ...
        # Sound button.
        if (self.x_min_sound_button <= mouse_x <= self.x_max_sound_button and
                self.y_min_sound_button <= mouse_y <= self.y_max_sound_button and
                not (self.show_welcome_page or self.show_rules_page or
                     self.show_options_page or self.show_credits_page or
                     self.show_computer_first_page or self.show_player_first_page or
                     self.show_rules_main_page)):
    ...
    def check_victory_condition(self):
        ...
        When victory is achieved, displays the victory page
        and stops the timer.

        Returns:
            bool:
                True if victory condition is met, False otherwise.
        """
        if (len(self.used_card_positions) == 15 or
                len(self.bot.cards) == 15):
    ...
    def check_failure_condition(self):
        ...
        Returns:
            bool:
                True if failure condition is met, False otherwise.
        """
        if (len(self.used_card_positions) == 0 or
                len(self.bot.cards) == 0):
    ...
    def handle_word_click(self, mouse_x, mouse_y):
        ...
        Manages the swapping of letters between selected card
        and current word,including handling of previous swaps
                ...
                if x <= mouse_x <= x + width and y <= mouse_y <= y + height:
                    if self.selected_card:
                        ...
                        if i in self.replaced_positions:
                            print(f"[handle_word_click] Position {i} has "
                                  f"already been replaced and cannot "
                                  f"be replaced again")
                            return False

                        # Check if the letters are the same as those in the original word.
                        if (previous_card.lower() ==
                                self.previous_word_cards[i].lower()):
                            print(f"[handle_word_click] Cannot replace same "
                                  f"card：{previous_card}")
                            self.notification.show_message_box(
                                "CANNOT SWAP SAME CARD"
                            )
                            return False
                        ...
                        # Handle previous swap.
                            ...
                            if self.last_swapped_position == i:
                                if self.used_card_positions:
                                    ...
                                    print(f"[handle_word_click] Restored "
                                          f"Previous Card at Position {last_card_position}")
                            ...
                            # Different position swap.
                                ...
                                if self.used_card_positions:
                                    last_card_position = self.used_card_positions.pop()
                                    print(f"[handle_word_click] Restored Card "
                                          f"at Position {last_card_position}")
                                print(f"[handle_word_click] Restored Position "
                                      f"{self.last_swapped_position} to "
                                      f"{self.original_cards[self.last_swapped_position]}")
                        ...
                        print(f"[handle_word_click] Card Swap confirmation: "
                              f"{current_card} -> {previous_card}")
                        print(f"[handle_word_click] Current Word: "
                              f"{''.join(self.word_cards)}")
    ...
    def check_word_validity(self):
        ...
        Uses GameFunctions' check_exists function to verify
        ...
        if self.logic.check_exists(current_word_str, previous_word_str):
            if self.side_status == 0:
                ...
                self.previous_word_cards = self.word_cards.copy()
                self.previous_used_card_positions = self.used_card_positions.copy()
                print(f"[check_word_validity] Answer checked (Player): "
                      f"{self.player_answer_status}")

            elif self.side_status == 1:
                if self.bot.current_turn_will_answer_or_not:
                    if not self.bot.current_turn_answer == None:
                        self.computer_answer_status = 1
                        print(f"[check_word_validity] Answer checked (Computer "
                              f"answered and not NONE): "
                              f"{self.computer_answer_status}")
                        self.notification.show_message_box("VALID WORD")
                    else:
                    ...
                    new_card = self.deck.pop()
                    self.bot.add_card(new_card)
                    print(f"[handle_bot_turn] Computer got a penalty card: "
                          f"{new_card}")
                    self.notification.show_message_box("NO WORD CHANGED")
            ...
            if self.side_status == 0:
                ...
                self.used_card_positions = (
                    self.previous_used_card_positions.copy()
                )
                print(f"[check_word_validity] Answer Checked (Player): "
                      f"{self.player_answer_status}")
    ...
    def handle_timer_event(self):
        ...
        if self.timer_seconds > 0 and not self.game_paused:
            ...
            if self.side_status == 0:
                ...
                print(f"[handle_timer_event] Previous Word:",
                      self.previous_word_cards)
                ...
                if not self.show_remove_page and not self.show_remove_page_mode:
                    if self.word_cards == self.previous_word_cards:
                        ...
                        print(f"[handle_timer_event] Answer Checked (Player "
                              f"did not change Word): {self.player_answer_status}")
                        self.notification.show_message_box("NO WORD CHANGED")
                        self.player_answer_status = 0
                        ...
                        if self.player_answer_status == 1:
                            ...
                            print(f"[handle_timer_event] Points (Player): "
                                  f"{self.points}")
                ...
                if self.computer_answer_status == 1:
                    ...
                    print(f"[handle_timer_event] Points (Computer): "
                          f"{self.computer_points}")

                self.bot.end_turn()

            print(f"[handle_timer_event] Side Status (Player-0 Computer-1): "
                  f"{self.side_status}")
            print(f"[handle_timer_event] Player's answer status "
                  f"(Invalid-0 Valid-1): {self.player_answer_status}")
            print(f"[handle_timer_event] Computer's answer status "
                  f"(Invalid-0 Valid-1): {self.computer_answer_status}")
            ...
            if (self.points == 0 or self.points == 1 or
                    self.points == 2 or self.points == 3):
    ...
    def handle_popup_click(self, pos):
        ...
        Returns:
            bool:
                True if the popup interaction was successful,
            False otherwise.
        """
        if self.show_popup:
            if self.popup_button_rect.collidepoint(pos):
                ...
                # Shuffle the deck.
                print(f"[handle_popup_click] Deck (before shuffled): "
                      f"{self.deck}")
                self.deck = self.logic.fisher_shuffle(self.deck)
                print(f"[handle_popup_click] Deck (after shuffled): "
                      f"{self.deck}")
                ...
                print(f"[handle_popup_click] Player's cards have been sorted: "
                      f"{valid_cards}")
                ...
                if self.star_card_active:
                    ...
                    self.previous_word_cards = self.word_cards.copy()
                    self.star_card_active = False
                    print(f"[handle_popup_click] New word generated ('*' Card): "
                          f"{self.word}")
                ...
                if self.side_status == 0:
                    ...
                    print(f"[handle_popup_click] Current Round "
                          f"{self.round_counter_popup_click_count / 2}")
            ...
            if self.popup_remove_yes_button_rect.collidepoint(mouse_x, mouse_y):
                if pygame.mouse.get_pressed()[0]:
                    ...
                    print(f"[handle_popup_click] Removal Successful\n"
                          f"[handle_popup_click] Remove Mode: Disable")
                    print(f"[handle_popup_click] --- Remove Mode Disables ---")

                    if (self.selected_card and
                            isinstance(self.selected_card[0], tuple)):
            ...
            if self.popup_bot_difficulty_easy_button_rect.collidepoint(
                    mouse_x, mouse_y):
                self.button_sound.play()
            ...
            elif self.popup_bot_difficulty_medium_button_rect.collidepoint(
                    mouse_x, mouse_y):
                self.button_sound.play()
            ...
            elif self.popup_bot_difficulty_hard_button_rect.collidepoint(
                    mouse_x, mouse_y):
                self.button_sound.play()
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            # Mouse click event.
                ...
                #Card click event.
                if (not self.show_welcome_page or self.show_rules_page or
                        self.show_options_page or self.show_credits_page):
                    self.handle_card_click(mouse_x, mouse_y)
                    self.handle_word_click(mouse_x, mouse_y)

                # Welcome page click.
                if (self.show_welcome_page or self.show_rules_page or
                        self.show_options_page or self.show_credits_page):
                    if (self.x_min_play_button <= mouse_x <=
                            self.x_max_play_button and
                            self.y_min_play_button <= mouse_y <=
                            self.y_max_play_button):
                        if (self.show_rules_page or self.show_options_page or
                            ...
                            if self.side_status == 0:
                                ...
                                print(f"[handle_events] Player's cards have been sorted: "
                                      f"{valid_cards}")
                    ...
                    elif (self.x_min_rules_button <= mouse_x <=
                          self.x_max_rules_button and
                          self.y_min_rules_button <= mouse_y <=
                          self.y_max_rules_button):
                        self.button_sound.play()
                        self.show_welcome_page = False
                    ...
                    elif (self.x_min_options_button <= mouse_x <=
                          self.x_max_options_button and
                          self.y_min_options_button <= mouse_y <=
                          self.y_max_options_button):
                        self.button_sound.play()
                        self.show_welcome_page = False
                        self.show_rules_page = False
                    ...
                    elif (self.x_min_credits_button <= mouse_x <=
                          self.x_max_credits_button and
                          self.y_min_credits_button <= mouse_y <=
                          self.y_max_credits_button):
                        self.button_sound.play()
                        self.show_welcome_page = False
                        self.show_rules_page = False
                        self.show_options_page = False
                ...
                # Popup click.
                elif (self.show_popup or self.show_popup_remove or
                      self.show_popup_bot_difficulty):
                    self.handle_popup_click(event.pos)

                # First role page click.
                elif ((self.show_computer_first_page or
                      self.show_player_first_page) and
                      not self.show_popup_bot_difficulty):
                    self.button_sound.play()
                    self.show_computer_first_page = False
                    self.show_player_first_page = False
                ...
                elif (not (self.show_welcome_page or self.show_rules_page or
                          self.show_options_page or self.show_credits_page) and
                      self.x_min_game_paused_page <= mouse_x <=
                      self.x_max_game_paused_page and
                      self.y_min_game_paused_page <= mouse_y <=
                      self.y_max_game_paused_page):
                    if (not self.show_game_paused_page and not (
                            self.show_computer_first_page and
                            self.show_player_first_page) and
                            not self.show_rules_main_page):
                        self.button_sound.play()
                        self.game_paused = True
                ...
                elif (self.show_game_paused_page and
                      self.x_min_rules_main_button <= mouse_x <=
                      self.x_max_rules_main_button and
                      self.y_min_rules_main_button <= mouse_y <=
                      self.y_max_rules_main_button):
                    self.button_sound.play()
                    ...
                    pygame.time.set_timer(self.timer_event, 0)
                          ...
                          self.show_options_page or self.show_credits_page):
                    if (self.x_min_rules_main_button <= mouse_x <=
                            self.x_max_rules_main_button and
                            self.y_min_rules_main_button <= mouse_y <=
                            self.y_max_rules_main_button and
                            not self.show_rules_page):
                        self.button_sound.play()
                        self.show_rules_main_page = True
                        self.show_game_paused_page = False
                        pygame.time.set_timer(self.timer_event, 0)
    ...
    def draw_round_counter(self):
        ...
        """
        round_text = self.font_round.render(
            str(self.round), True, self.color_text_round
        )
    ...
    def draw(self):
        ...
        if self.show_options_page:
            ...
            if self.options_click_sound == 1:
                self.marker_click_sound_enable = 1
                pygame.draw.rect(
                    self.screen, self.color_option_button,
                    self.options_box1_rect
                )
            else:
